<link href="http://orders.enterprise-esolutions.com/keyboard/css/jquery-ui.min.css" rel="stylesheet">
    ...
    <div class="modal-dialog" style="width:auto;max-width:800px">
        <div class="modal-content">
             ...
             <div class="modal-body">
              ...
              <input name="data[form][date]" type="hidden" value="2017-01-20" id="inputdate"></input>
              <?php echo $this->Form->end(); ?>
<?php 
/*
  echo '<center><table style="width:200px;" ><tr>';
  echo $this->Form->create('form',array('style'=>'margin-top:30px;margin-bottom:-30px','id'=>'DateForm'));  

   echo '<td><div class="input-group margin">';
  echo $this->Form->input('date',array('id'=>'datepicker','label'=>'','style'=>'width:150px!important;height:34px!important;margin-bottom:3px!important;border: 2px solid #ccc','dateFormat'=>'Y-m-d'    ,'placeholder'=> $today     ));
  echo '<span class="input-group-btn">
                                            <button class="btn btn-success btn-flat" type="submit"><i class="fa fa-eye"></i> View date</button>
                                        </span>
                                    </div></td>'; 
   echo  '<td style="margin-left:50px;">'.$this->Form->button('<i class="fa  fa-calendar-o"></i> Today',array('class'=>'btn btn-primary btn-flat','type'=>'submit','name'=>'Today' ,'label'=>''  )).'</td> ';  
   echo  '<td style="margin-left:50px;">'.$this->Form->button('<i class="fa  fa-calendar"></i> Coming',array('class'=>'btn btn-warning btn-flat','type'=>'submit','name'=>'Coming','label'=>''  )).'</td> ';  
...
// check if current order had suborders
  ...
  echo '<tr ><th class="orderselect" order="'.$idor.'" style="'.$cbgcolor.'color:#555555!important;">'.$dispID.' </th><th class="orderselect" order="'.$idor.'" style="'.$cbgcolor.'text-align:center;color:#555555!important;">'.$ordertime.'</th><th class="orderselect" order="'.$idor.'" style="'.$cbgcolor.'text-align:center;color:#555555!important;"><span style="color:black;font-weight:bold;"></span></br>    . XX .   </th>';
...
$('#checkout').click(function(){
 ...
 // Form confirm (yes/ok) handler, submits form /
  $('#confirmCheckoutOrder').find('.modal-footer #confirmcheckout').on('click', function(){
      //$(this).data('form').submit();
       $.get("/orders/checkout/"+order).complete(function()
            {location.reload(true);});
